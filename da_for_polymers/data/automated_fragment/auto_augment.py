...
def fragment_SMILES(fragmented: list[str]) -> str:
    """Return the fragmented SMILES string.

    Args:
        fragmented: A list of polymer fragments.

    Returns:
        A SMILES string of the polymer molecule.
    """
    return ".".join(fragmented)


def shuffled_SMILES(shuffled: list[list[str]]) -> list[str]:
    """Return the shuffled SMILES strings.

    Args:
        shuffled: A list of shuffled polymer molecules.

    Returns:
        A list of shuffled SMILES strings.
    """
    shuffled_smiles = []
    for i in range(len(shuffled)):
        shuffled_smiles.append(".".join(shuffled[i]))
...
def augment_dataset(dataset: Path, augmented_dataset: Path) -> pd.DataFrame:
    ...
    dataset_df["polymer_automated_frag_SMILES"] = fragment_mols.apply(
        lambda x: fragment_SMILES(x)
    )
    shuffled: list[list[str]] = fragment_mols.apply(lambda x: iterative_shuffle(x))
    dataset_df["polymer_automated_frag_aug"] = shuffled
    shuffled_smiles: list[str] = shuffled.apply(lambda x: shuffled_SMILES(x))
    dataset_df["polymer_automated_frag_aug_SMILES"] = shuffled_smiles
