...
def recursively_find_sidechains(mol:Chem.Mol, sidechains_idx: list, current_idx: int, current_fragment: dict, backbone_idx: list, asterick_idx: list) -> list:
    ...
    # recursive case
        ...
        if len(atom.GetNeighbors()) == 1 and current_idx not in backbone_idx and current_idx not in current_fragment["sidechain"] and current_idx not in current_fragment["backbone"] and current_idx not in asterick_idx:
            sidechains_idx.append(current_idx)
            return recursively_find_sidechains(mol, sidechains_idx, current_idx, current_fragment, backbone_idx, asterick_idx)
            ...
            if atom.IsInRing():
                ...
                if all_atoms_in_ring_added:
                    ...
                    for neighbor in atom.GetNeighbors():
                        ...
                        if neighbor.GetIdx() not in sidechains_idx and neighbor.GetIdx() not in backbone_idx and neighbor.GetIdx() not in current_fragment["sidechain"] and neighbor.GetIdx() not in current_fragment["backbone"] and neighbor.GetIdx() not in asterick_idx:
                            sidechains_idx.append(current_idx)
                            return recursively_find_sidechains(mol, sidechains_idx, neighbor.GetIdx(), current_fragment, backbone_idx, asterick_idx)
    ...
    for b_idx in backbone_idx:
        ...
        for neighbor in atom.GetNeighbors():
            ...
            if atom.IsInRing():
                ...
                base_fragments[current_unit_idx]["backbone"]: list = list(dict.fromkeys(base_fragments[current_unit_idx]["backbone"]))
                ...
                for ring_backbone_idx in base_fragments[current_unit_idx]["backbone"]:
                    ...
                    for r_neighbor in ring_backbone_mol.GetNeighbors():
                        if r_neighbor.GetIdx() not in base_fragments[current_unit_idx]["backbone"] and r_neighbor.GetIdx() not in backbone_idx:
                            ...
                            if recursively_find_sidechains(mol, [], r_neighbor.GetIdx(), base_fragments[current_unit_idx], backbone_idx, asterick_idx) is not None:
                                base_fragments[current_unit_idx]["sidechain"].extend(recursively_find_sidechains(mol, [], r_neighbor.GetIdx(), base_fragments[current_unit_idx], backbone_idx, asterick_idx))
                # Find connections
                if neighbor.GetIdx() in backbone_idx and neighbor.GetIdx() not in base_fragments[current_unit_idx]["backbone"]:
                    base_fragments[current_unit_idx]["connections"].append(neighbor.GetIdx())
            else:
                if (
                    neighbor.GetIdx() not in backbone_idx
                    and neighbor.GetIdx() not in asterick_idxs
                ):
                    base_fragments[current_unit_idx]["sidechain"].append(neighbor.GetIdx())
                # New fragment
                elif neighbor.GetIdx() in backbone_idx and neighbor.GetIdx() not in base_fragments[current_unit_idx]["backbone"]:
                    base_fragments[current_unit_idx]["connections"].append(neighbor.GetIdx())
                
    return mol, base_fragments

def fragment_mol_from_indices(mol: Chem.Mol, base_fragments: list[dict]) -> list[Chem.Mol]:
    """Fragment a polymer molecule into its monomer units.
    ...
    new_mol_smi: str = Chem.MolToSmiles(new_mol)
    new_frags: list[str] = new_mol_smi.split(".") # split on "." to get fragments
...
def augment_dataset(dataset: Path, augmented_dataset: Path) -> pd.DataFrame:
    ...
    indices: tuple[Chem.Mol, dict] = dataset_df.apply(lambda x: get_fragment_indices(x["smiles"]), axis=1)
    fragment_mols = indices.apply(lambda x: fragment_mol_from_indices(x[0], x[1]))
    shuffled = fragment_mols.apply(lambda x: iterative_shuffle(x))
    dataset_df["augmented_polymer"] = shuffled
