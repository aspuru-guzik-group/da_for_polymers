...
CORRELATION_PLOT = pkg_resources.resource_filename(
    "da_for_polymers", "data/exploration/PV_Wang/pv_correlation_plot.png"
)

CORRELATION_HEATMAP_PLOT = pkg_resources.resource_filename(
    "da_for_polymers", "data/exploration/PV_Wang/pv_correlation_heatmap.png"
)

RMSE_HEATMAP_PLOT = pkg_resources.resource_filename(
    "da_for_polymers", "data/exploration/PV_Wang/pv_rmse_heatmap.png"
)
...
class Correlation:
    ...
    def heatmap_plot(self, columns_list_idx, option):
        ...
        for pair in permutations:
            ...
            while index < len(column_0):
                if not isna_column_0[index] and not isna_column_1[index]:
                    filtered_column_0.append(float(column_0[index]))
                    filtered_column_1.append(float(column_1[index]))
                index += 1
            print(type(filtered_column_0[0]))
            # handle different data types (str, float)
            if (
                isinstance(filtered_column_0[0], float)
                and isinstance(filtered_column_1[0], float)
                or isinstance(filtered_column_0[0], int)
                and isinstance(filtered_column_1[0], int)
            ):
                if option == "r":
                    # find correlation coefficient
                    result = np.corrcoef(
                ...
                heatmap_array[column_idx_dict[pair[0]]][
                    column_idx_dict[pair[1]]
                ] = round(result, 2)
            else:
                heatmap_array[column_idx_dict[pair[0]]][column_idx_dict[pair[1]]] = nan
...
corr_plot.heatmap_plot([2, 3, 5, 6, 7, 8, 9, 10], "r")
