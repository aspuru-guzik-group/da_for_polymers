...
FP_SWELLING = pkg_resources.resource_filename(
    "da_for_polymers", "data/input_representation/Swelling_Xu/fingerprint/swelling_fingerprint.csv"
)

BRICS_FRAG_DATA = pkg_resources.resource_filename(
    "da_for_polymers", "data/input_representation/Swelling_Xu/BRICS/master_brics_frag.csv"
)
...
def cli_main():
    ...
    for i in seed:
        ...
        for train_ix, test_ix in cv_outer.split(x, dataset.data["Polymer"]):
            ...
            # evaluate the model
            corr_coef = np.corrcoef(y_test, yhat_test)[0, 1]
