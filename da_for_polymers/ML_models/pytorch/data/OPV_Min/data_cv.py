...
from da_for_polymers.ML_models.pytorch.data.OPV_Min.tokenizer import Tokenizer
import torch
...
class OPVDataset(Dataset):
    ...
    def get_splits(self, seed_val, n_test=0.10, test_test=0.10):
        # train = 80%, val = 10%, test = 10%
        total_size = len(self.x)
        test = round(total_size * n_test)
        val = round(total_size * test_test)
    ...
    def get_splits_cv(self, kth_fold, k_fold=5):
        ...
        for train_ix, test_ix in cv_outer.split(self.x):
            if kth_fold == fold_count:
                num_of_val = int(len(test_ix) * test_proportion)
                test_ix = np.random.choice(test_ix, num_of_val, replace=False)
                test_ix = np.setdiff1d(test_ix, test_ix)
                train = Subset(self, train_ix)
                val = Subset(self, test_ix)
...
class OPVDataModule(pl.LightningDataModule):
    ...
    def prepare_transformer(self):
        ...
        if self.smiles and self.input == 0:
            ...
            # test_df = pd.DataFrame(self.pce_val.dataset.x.numpy())
